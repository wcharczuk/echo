...
func (am *AuthManager) Login(userID string, context *Ctx) (session *Session, err error) {
	...
	if am.persistHandler != nil {
		err = am.persistHandler(context, session, Tx(context))
		if err != nil {
			return nil, err
		}
	}
...
func (am *AuthManager) Logout(session *Session, context *Ctx) error {
	...
	if am.removeHandler != nil {
		if context != nil {
			return am.removeHandler(session.SessionID, Tx(context))
		}
...
func (am *AuthManager) VerifySession(context *Ctx) (*Session, error) {
	...
	var session *Session
	if context != nil {
		session, err = am.fetchHandler(sessionID, Tx(context))
	} else {
	...
	if am.validateHandler != nil {
		if context != nil {
			err = am.validateHandler(session, Tx(context))
		} else {
			err = am.validateHandler(session, nil)
		}
		if err != nil {
			return nil, err
		}
